...
import { X, Plus, DollarSign, User } from 'lucide-react';
...
interface SessionFormGlobalProps {
  clients: Client[];
  onSave: (session: Omit<Session, 'id' | 'createdAt'>) => void;
  onCancel: () => void;
}

export function SessionFormGlobal({ clients, onSave, onCancel }: SessionFormGlobalProps) {
  const [formData, setFormData] = useState({
    ...
    summary: '',
    actionItems: [''],
    sessionType: 'Free' as 'Free' | 'Paid' | 'Chemistry',
    dueAmount: '',
    currency: 'USD'
  });

  const getNextSessionNumber = (clientId: string) => {
    // This would need to be calculated based on existing sessions for that client
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.clientId && formData.focusArea && formData.summary) {
      const sessionData: Omit<Session, 'id' | 'createdAt'> = {
        clientId: formData.clientId,
        sessionNumber: getNextSessionNumber(formData.clientId),
    ...
    <div className="max-w-2xl mx-auto">
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">Create a new coaching session</p>
        </CardHeader>
        <CardContent>
